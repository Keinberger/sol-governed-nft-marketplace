    ...
    : describe(contractName, () => {
          ...
          describe("removePaymentTokenAtListing", () => {
              ...
              it("reverts if market not open", async () => {
                  await contract.setState(1)
                  await expect(
                      contract.removePaymentTokenAtListing(nftContract.address, "0", "0")
                  ).to.be.revertedWith("NftMarketplace__StateIsNot(2)")
              })
              it("reverts if caller is not nft owner", async () => {
                  await expect(
                      userContract.removePaymentTokenAtListing(nftContract.address, "0", "0")
                  ).to.be.revertedWith(
                      `NftMarketplace__NotOwnerOfNft("${nftContract.address}", ${"0"})`
                  )
              })
              it("reverts if nft not listed", async () => {
                  ...
                  await expect(
                      contract.removePaymentTokenAtListing(nftContract.address, "1", "0")
                  ).to.be.revertedWith(
                      `NftMarketplace__NftNotListed("${nftContract.address}", ${"1"})`
                  )
              })
              it("reverts if payment token not listed", async () => {
                  await expect(
                      contract.removePaymentTokenAtListing(nftContract.address, "0", "2")
                  ).to.be.revertedWith("NftMarketplace__IndexOutOfBounds()")
              })
              it("removes payment token at listing", async () => {
                  await contract.removePaymentTokenAtListing(nftContract.address, "0", "1")
                  const listing = await contract.getListing(nftContract.address, "0")
                  expect(
                      listing.paymentTokenAddresses[listing.paymentTokenAddresses.length - 1]
                  ).to.equal(basicToken.address)
              })
              it("emits event correctly", async () => {
                  await expect(contract.removePaymentTokenAtListing(nftContract.address, "0", "1"))
                      .to.emit(contract, "NftPaymentTokenRemoved")
                      .withArgs(nftContract.address, "0", governanceToken.address)
              })
          })
          describe("buyNftEth", () => {
